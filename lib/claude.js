...
- RPI (Refinery Pressure Index): Infrastructure strikes + corridor traffic + export capacity. Higher = more fiscal pressure on occupation funding. Current value in current_scores.rpi_current.RPI.
- CCI (Civilian Confidence Index): Fuel rationing + food shortages + panic purchasing + grey market. Higher = more acute civilian distress. Current value in current_scores.cci_current.CCI.
- OFP (Occupation Financial Pressure): Export revenue + NWF liquid assets + OFZ debt stress + regional budget cuts + shadow fleet stress. Higher = more fiscal strain on occupation. Current value in current_scores.ofp_current.OFP.
- ECS = (RPI×0.5) + (CCI×0.5)
...
- If a dial value is not present in current_scores, say "not present in context" rather than estimating it.`;
